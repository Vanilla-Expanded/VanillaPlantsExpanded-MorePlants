...
{
	public class WorkGiver_GrowerSowFlowerGarden : WorkGiver_GrowerFlowerGarden
    {
		...
		protected override bool ExtraRequirements(IPlantToGrowSettable settable, Pawn pawn)
		{
            ...
            Zone_GrowingFlowerGarden zone_Growing = settable as Zone_GrowingFlowerGarden;
			IntVec3 c;
            ...
            WorkGiver_GrowerFlowerGarden.wantedPlantDef = WorkGiver_GrowerFlowerGarden.CalculateWantedPlantDef(c, pawn.Map);
			return WorkGiver_GrowerFlowerGarden.wantedPlantDef != null;
		}
		...
		public override Job JobOnCell(Pawn pawn, IntVec3 c, bool forced = false)
		{
			...
			if (WorkGiver_GrowerFlowerGarden.wantedPlantDef == null)
			{
                WorkGiver_GrowerFlowerGarden.wantedPlantDef = WorkGiver_GrowerFlowerGarden.CalculateWantedPlantDef(c, map);
				if (WorkGiver_GrowerFlowerGarden.wantedPlantDef == null)
				{
					return null;
				}
            ...
            Zone_GrowingFlowerGarden zone_Growing = c.GetZone(map) as Zone_GrowingFlowerGarden;
			bool flag = false;
			for (int i = 0; i < thingList.Count; i++)
			{
				Thing thing = thingList[i];
				if (thing.def == WorkGiver_GrowerFlowerGarden.wantedPlantDef)
				{
					return null;
				}
			...
			if (WorkGiver_GrowerFlowerGarden.wantedPlantDef.plant.cavePlant)
			{
				if (!c.Roofed(map))
				{
			...
			if (WorkGiver_GrowerFlowerGarden.wantedPlantDef.plant.interferesWithRoof && c.Roofed(pawn.Map))
			{
				return null;
			}
			...
			Thing thing2 = PlantUtility.AdjacentSowBlocker(WorkGiver_GrowerFlowerGarden.wantedPlantDef, c, map);
			if (thing2 != null)
			{
				...
				if (plant2 != null && pawn.CanReserveAndReach(plant2, PathEndMode.Touch, Danger.Deadly, 1, -1, null, forced) && !plant2.IsForbidden(pawn))
				{
					...
					{
                        Zone_GrowingFlowerGarden zone_Growing2 = c.GetZone(map) as Zone_GrowingFlowerGarden;
                        Zone_GrowingFlowerGarden zone_Growing3 = plant2.Position.GetZone(map) as Zone_GrowingFlowerGarden;
			...
			if (WorkGiver_GrowerFlowerGarden.wantedPlantDef.plant.sowMinSkill > 0 && pawn.skills != null && pawn.skills.GetSkill(SkillDefOf.Plants).Level < WorkGiver_GrowerFlowerGarden.wantedPlantDef.plant.sowMinSkill)
			{
				JobFailReason.Is("UnderAllowedSkill".Translate(WorkGiver_GrowerFlowerGarden.wantedPlantDef.plant.sowMinSkill), def.label);
				return null;
			}
			...
			if (!WorkGiver_GrowerFlowerGarden.wantedPlantDef.CanNowPlantAt(c, map) || !PlantUtility.GrowthSeasonNow(c, map, forSowing: true) || !pawn.CanReserve(c, 1, -1, null, forced))
			{
				return null;
			}
			Job job = JobMaker.MakeJob(JobDefOf.Sow, c);
			job.plantDefToSow = WorkGiver_GrowerFlowerGarden.wantedPlantDef;
			return job;
		}
	}
}