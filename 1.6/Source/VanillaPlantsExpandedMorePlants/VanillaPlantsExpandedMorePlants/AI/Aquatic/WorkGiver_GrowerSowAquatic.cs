...
{
	public class WorkGiver_GrowerSowAquatic : WorkGiver_GrowerAquatic
	{
		...
		protected override bool ExtraRequirements(IPlantToGrowSettable settable, Pawn pawn)
		{
			...
			Zone_GrowingAquatic zone_Growing = settable as Zone_GrowingAquatic;
			IntVec3 c;
			...
			WorkGiver_GrowerAquatic.wantedPlantDef = WorkGiver_GrowerAquatic.CalculateWantedPlantDef(c, pawn.Map);
			return WorkGiver_GrowerAquatic.wantedPlantDef != null;
		}
		...
		public override Job JobOnCell(Pawn pawn, IntVec3 c, bool forced = false)
		{
			...
			for (int i = 0; i < thingList.Count; i++)
			{
				...
				if (thing.def == WorkGiver_GrowerAquatic.wantedPlantDef)
				{
					return null;
				}
			...
			Thing thing2 = PlantUtility.AdjacentSowBlocker(WorkGiver_GrowerAquatic.wantedPlantDef, c, map);
			if (thing2 != null)
			{
				...
				if (plant2 != null && pawn.CanReserveAndReach(plant2, PathEndMode.Touch, Danger.Deadly, 1, -1, null, forced) && !plant2.IsForbidden(pawn))
				{
					...
					{
						Zone_GrowingAquatic zone_Growing2 = c.GetZone(map) as Zone_GrowingAquatic;
						Zone_GrowingAquatic zone_Growing3 = plant2.Position.GetZone(map) as Zone_GrowingAquatic;
			...
			if (WorkGiver_GrowerAquatic.wantedPlantDef.plant.sowMinSkill > 0 && pawn.skills != null && pawn.skills.GetSkill(SkillDefOf.Plants).Level < WorkGiver_GrowerAquatic.wantedPlantDef.plant.sowMinSkill)
			{
				JobFailReason.Is("UnderAllowedSkill".Translate(WorkGiver_GrowerAquatic.wantedPlantDef.plant.sowMinSkill), def.label);
				return null;
			}
			...
			if (!WorkGiver_GrowerAquatic.wantedPlantDef.CanNowPlantAt(c, map) || !PlantUtility.GrowthSeasonNow(c, map, wantedPlantDef) || !pawn.CanReserve(c, 1, -1, null, forced))
			{
				return null;
			}
			Job job = JobMaker.MakeJob(JobDefOf.Sow, c);
			job.plantDefToSow = WorkGiver_GrowerAquatic.wantedPlantDef;
			return job;
		}
	}
}